...
// Define styles for GTC (same as Global Digital Connect structure)
...
interface GTCTaxableInvoicePDFProps {
  companyName: string;
...
// Document component for GTC
const GTCTaxableInvoiceDocument = (props: GTCTaxableInvoicePDFProps) => {
  const totalInWords = numberToWords(props.total);
    ...
    <Document>
      <Page size="A4" style={props.fitToOnePage ? styles.pageCompact : styles.page}>
        ...
        <View style={styles.header}>
          <Text style={styles.companyName}>{GTC_COMPANY_DETAILS.name}</Text>
          <Text style={styles.companyDetails}>
            {GTC_COMPANY_DETAILS.address}
          </Text>
          <Text style={styles.companyDetails}>
            GST: {GTC_COMPANY_DETAILS.gst} | Proprietor: {GTC_COMPANY_DETAILS.proprietor}
          </Text>
        </View>
        ...
        {/* Bank Details - GTC specific */}
        <View style={styles.bankDetails}>
          <Text style={styles.bankTitle}>Company's Bank Details</Text>
          <Text style={styles.bankInfo}>A/c Holder's Name: Global Trading Corporation</Text>
          <Text style={styles.bankInfo}>Bank Name: ICICI BANK LIMITED</Text>
          <Text style={styles.bankInfo}>A/c No.: 734805500123</Text>
          <Text style={styles.bankInfo}>Branch & IFS Code: AMLIDIH RAIPUR & ICIC0007348</Text>
        </View>
          ...
          <View style={styles.rightSignature}>
            <View style={{ alignItems: "center" }}>
              ...
              <Image
                ...
                src={getGTCSignatureImage()}
              />
              <Text style={{ fontSize: 10, fontWeight: "bold" }}>Authorized Signatory</Text>
              <Text style={{ fontSize: 9 }}>{GTC_COMPANY_DETAILS.name}</Text>
            </View>
          </View>
        </View>
...
const GTCTaxableInvoicePDF = forwardRef<{ downloadPDF: () => Promise<void> }, GTCTaxableInvoicePDFProps>((props, ref) => {
  useImperativeHandle(ref, () => ({
    downloadPDF: async () => {
      const blob = await pdf(<GTCTaxableInvoiceDocument {...props} />).toBlob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `gtc-taxable-invoice-${props.invoiceNumber || 'draft'}.pdf`;
      link.click();
  ...
  return (
    <ClientOnlyPDFViewer>
      <PDFViewer style={{ width: '100%', height: '600px' }}>
        <GTCTaxableInvoiceDocument {...props} />
      </PDFViewer>
    </ClientOnlyPDFViewer>
  );
});

GTCTaxableInvoicePDF.displayName = 'GTCTaxableInvoicePDF';

export default GTCTaxableInvoicePDF;
